...
def all_pairs_matching_predicate(values, pred):
    ...
    Examples
    --------
    >>> from zipline._testing import all_pairs_matching_predicate
...
def subtest(iterator, *_names):
    ...
    Consider using ``zipline._testing.parameter_space`` when subtests
    ...
    * Have a large parameter space we are testing
      (see _tests/utils/test_events.py).
    ...
    See Also
    --------
    zipline._testing.parameter_space
    """
    def dec(f):
        @wraps(f)
...
def test_resource_path(*path_parts):
    return os.path.join(zipline_git_root, 'zipline', '_tests', 'resources', *path_parts)
